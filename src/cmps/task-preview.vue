<template>
   ...
   <router-link class="list-card" :to="`/board/${board._id}/${group.id}/${task.id}`">
      <!-- cover -->
      <div v-if="isCoverBgc" class="list-card-cover" :style="{ backgroundColor: task.style.bgColor, height: isHeight + 'px' }">
         <span class="icon-edit icon-sm list-card-operation"></span>
      </div>
      ...
      <div class="list-card-details">
         <div v-if="task.labels && task.labels.length" class="list-card-labels">
            <label-checkboxes :checked="checked" :toggle-checkbox="toggleCheckbox" />
            <label for="label-toggler" class="label-toggler">
               <span v-for="label in task.labels" :key="label.id" class="card-label"
...
<script>
   ...
   export default {
      ...
      data() {
         return {
            labelsAreOpen: true,
            currLoggedUser: null, required: true,
            checked: true
         }
      },
      created() {},
